    ...
    render() {
        ...
        this.innerHTML = `
            <figure class="gap-2 text-sm items-center flex flex-col text-gray-400 transform hover:scale-105 hover:text-white transition ease-out">
                <div style="background-image: url(${this._movie.poster_path ? `https://image.tmdb.org/t/p/w200/${this._movie.poster_path}` : "https://upload.wikimedia.org/wikipedia/commons/f/fc/No_picture_available.png"});" class="flex justify-between bg-cover w-40 h-60 rounded-lg card-shadow hover:shadow-lg" alt=${this._movie.title}>
                ...
                <figcaption class="text-sm text-center w-44"><a href='./details.html?id=${this._movie.id}'>${this._movie.title || "No Caption"}</a></figcaption>
            </figure>
        `;
    }
}
